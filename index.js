...
// 1. Homepage & Status
app.get('/', (req, res) => {
    res.send("<body style='background:#121212;color:white;font-family:sans-serif;text-align:center;'><h1>🏦 Emari Bank Vault: ONLINE</h1><p>Phase C Logic Active</p></body>");
});
...
app.post('/bank', (req, res) => {
    ...
    switch (action) {
        ...
        case "ISSUE_CARD":
            acc.hasCard = 1;
            acc.logs.push(`[${new Date().toLocaleDateString()}] Card Issued`);
            saveDB(); res.send("CARD_ISSUED"); break;

        case "DEPOSIT":
            acc.checking += parseInt(value);
            acc.logs.push(`[${new Date().toLocaleDateString()}] +L$${value} Dep`);
        ...
        case "WITHDRAW":
            ...
            if (acc.checking >= wAmt) {
                acc.checking -= wAmt;
                acc.logs.push(`[${new Date().toLocaleDateString()}] -L$${wAmt} With`);
                saveDB(); res.send("SUCCESS");
            } else res.send("INSUFFICIENT"); break;

        case "GET_LOGS":
            res.send("LOGS|" + (acc.logs.slice(-5).join("\n") || "No history.")); break;
...
// 3. The Card Image Generator
app.get('/card/:uuid', (req, res) => {
    const acc = accounts[req.params.uuid] || { checking: 0 };
    const canvas = createCanvas(512, 256);
    const ctx = canvas.getContext('2d');
    
    ctx.fillStyle = '#1a1a2e'; ctx.fillRect(0, 0, 512, 256); // BG
    ctx.fillStyle = '#ff3e6c'; ctx.font = 'bold 30px Arial'; ctx.fillText("EMARI MEGA BANK", 30, 50);
    ctx.fillStyle = '#ffffff'; ctx.font = '18px Courier'; ctx.fillText("HOLDER: " + req.params.uuid.substring(0, 15), 30, 180);
    ctx.fillStyle = '#00ffcc'; ctx.font = 'bold 28px Arial'; ctx.fillText("BAL: L$" + acc.checking, 30, 220);
